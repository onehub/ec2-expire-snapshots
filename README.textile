...
- @--filter@ := Instead of volume ids as the arguments, arguments are interpreted as filter clauses that AWS understands for DescribeSnapshots. See https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeSnapshots.html for filter parameters. You can use this option to find snapshots that are tagged in a particular way, for example: :=  @--filter tag-value=db-production-daily-snap@ := Would find all of your own snapshots that are tagged with db-production-daily-snap regardless of which volume they use. If you specify the flag, but provide no filter, all of your own snapshots will be returned.
